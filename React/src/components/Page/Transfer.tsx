  ...
  return (
    <HistoryAndTransferContainer>
      <div className="section">
        ...
        {dataList1 &&
          dataList1.slice(0, 2).map((item, index) => {
            ...
            return groupNames.map((groupName, i) =>
              groupName != "null" && money[i] != "null" ? (
                <div key={i} className="row">
                  ...
                  <div>${money[i]}</div>
                  <div>
                    <button onClick={() => handleAcceptTransfer(groupName)}>
                      轉帳
                    </button>
                  </div>
                </div>
      ...
      <div className="section">
        ...
        {dataList2 &&
          dataList2.slice(0, 2).map((item, index) => {
            const groupNames = item.groupName.split(",");
            const money = item.money.split(",");
            return groupNames.map((groupName, i) =>
              groupName != "null" && money[i] != "null" ? (
                <div key={i} className="row">
                  ...
                  <div>${money[i]}</div>
                  <div>
                    <button onClick={() => handleAccept(groupName)}>
                      接受
                    </button>
                    <button onClick={() => handleReject(groupName)}>
                      拒絕
                    </button>
                  </div>
                </div>
